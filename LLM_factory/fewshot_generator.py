...
def generic_create_fewshots(student_info,
    ...
    if data_mode == "onehot":
        return generic_create_onehot_fewshots(student_info, test_exercise_info, extra_datas, fewshots_num, fewshots_strategy, prompts, generate_analysis_func, generate_explanation_func, generate_analysis, generate_explanation, dataset_name, knowledge_graph_path)
    elif data_mode == "sparse":
        return generic_create_sparse_fewshots(student_info, test_exercise_info, extra_datas, fewshots_num, fewshots_strategy, prompts, generate_analysis_func, generate_explanation_func, generate_analysis, generate_explanation, dataset_name, knowledge_graph_path)
    elif data_mode == "moderate":
        return generic_create_moderate_fewshots(student_info, test_exercise_info, extra_datas, fewshots_num, fewshots_strategy, prompts, generate_analysis_func, generate_explanation_func, generate_analysis, generate_explanation, dataset_name, knowledge_graph_path)
    elif data_mode == "rich":
        raise NotImplementedError
    else:
...
def generic_create_onehot_fewshots(student_info, test_exercise_info, extra_datas, fewshots_num, fewshots_strategy,
                                   ...
                                   generate_analysis = True, generate_explanation = True,
                                   dataset_name = None, knowledge_graph_path = None) -> List[str]:
    # new create_fewshots function for llmsforkt
    ret_fewshots = []
    # fs_ex_ids = list of tuples (idex in student_info['exercises_logs'], exercise_id)
    fs_ex_ids = sample_fs_id(student_info['exercises_logs'], test_exercise_info, extra_datas, fewshots_num, fewshots_strategy, student_info, dataset_name, knowledge_graph_path)
    if fewshots_num == 0:
        return ret_fewshots
    first = True
        ...
        if first:
            ...
            if generate_explanation:
                ...
                try:
                    ...
                    print(f'[WARN] explanation failed for exercise {ex_id}, skipping: {e}')
        ret_fewshots.append(fewshot)

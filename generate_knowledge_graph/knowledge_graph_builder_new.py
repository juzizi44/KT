...
def _load_rows(
    ...
    Args:
        ...
        exclude_first_n_students: Number of students to skip from the beginning (to avoid data leakage)
    ...
    with recordings_path.open("r", encoding="utf-8") as f:
        for student_idx, line in enumerate(f):
            # Skip first N students to avoid data leakage
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of students to skip from the beginning of recordings.jsonl (to avoid data leakage). "
             "For example, if test_num=10 in main.py, set this to 10 to exclude test students.",
    )
    parser.add_argument(
        "--min-edge-weight",
        type=int,

...
DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL", os.getenv("OPENAI_BASE_URL", "https://api.bianxie.ai/v1"))
...
def deepseek_chat(
    ...
    start_ts = time.time()
    print(f"  [DeepSeek API] -> model={model}, msgs={len(messages)}, max_tokens={max_tokens}", flush=True)
    try:
        ...
        print(f"  [DeepSeek API] !! request failed after {elapsed:.1f}s: {exc}", flush=True)
        raise

    elapsed = time.time() - start_ts
    print(f"  [DeepSeek API] <- done in {elapsed:.1f}s, choices={len(response.choices)}", flush=True)
...
class DeepSeekChat(LLMModelBase):
    """通过 bianxie.ai 的 OpenAI 兼容接口调用 deepseek 模型。"""
    ...
    def generate_chat(self, messages: List[Message], max_tokens: int = 1024, temperature: float = 0.2, num_comps: int = 1) -> Union[List[str], str]:
        return deepseek_chat(self.name, messages, max_tokens, temperature, num_comps)
    ...
    def generate_prediction(self, fewshots: List[str], user_data: str, prompts: dict, use_selected_fewshot: bool = True) -> Union[List[str], str]:
        ...
        return deepseek_chat(self.name, prediction_messages, num_comps=1)
    ...
    def generate_analysis(self, fewshots: List[str], prompts: dict) -> Union[List[str], str]:
        ...
        return deepseek_chat(self.name, analysis_messages, num_comps=1)
    ...
    def generate_explaination(self, fewshots: List[str], prediction, prompts: dict) -> Union[List[str], str]:
        ...
        return deepseek_chat(self.name, explaination_messages, num_comps=1)
    ...
    def create_fewshots(self, student_info, test_exercise_info, extra_datas, fewshots_num, fewshots_strategy, data_mode, prompts, dataset_name=None, knowledge_graph_path=None) -> List[str]:
...
class DeepSeekFlash(DeepSeekChat):
    def __init__(self):

    ...
    def load_user_data(self, data_path: str, is_shuffle: bool, train_split: float, create_train_test = True) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if not create_train_test:
            ...
            if self.test_num != -1:
                ...
                needed_ids = set(test_data["student_id"])
                train_rows = []
                ...
                self.logger.write(f"Fast load: {len(test_data)} test students, {len(train_data)} matched train students")
            else:
            ...
            train_rows = []
            test_rows = []
            for i, row in tqdm.tqdm(recordings_df.iterrows(), total=len(recordings_df)):
                ...
                if is_shuffle:
                    ...
                    test_idx = [j for j in idx if j not in train_idx]
                else:
                ...
                train_rows.append({"student_id": student_id, "exercises_logs": exercises_logs[train_idx].tolist(), "is_corrects": is_corrects[train_idx].tolist()})
                test_rows.append({"student_id": student_id, "exercises_logs": exercises_logs[test_idx].tolist(), "is_corrects": is_corrects[test_idx].tolist()})
